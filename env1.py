...
class Env():
    ...
    def makestate(self, s):
        ...
        # st = s[:, [0, 2, 3, 7, 8, 9]]
        # minn = np.min(st, axis=0)
        # maxx = np.max(st, axis=0)
        # for i,xx in enumerate(maxx-minn):
        #     if xx==0:
        #         st[:,i] = st[:,i] - minn[i]
        #     else :
        #         st[:,i] = (st[:,i]-minn[i])/(maxx[i]-minn[i])
        # s[:, [0, 2, 3, 7, 8, 9]] = st
        # u[0] = u[0] / 4.
        # u[1] = 1.
        # u[2] = u[2] / 4.
        ...
        s = s.flatten()
        # s = np.insert(s, len(s), self.userinfo)
        return s
